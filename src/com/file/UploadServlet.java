...
public class UploadServlet extends HttpServlet {
	...
	public void processUploadedFile(FileItem item) {
		...
		System.out.println("文件(" + fileName+")下载完毕!");
		// 创建File对象,将上传的文件保存到C:\\upload文件夹下
File fDir = new File("F:/javaweb实验下载文件夹");
		if (!fDir.exists()) {
			fDir.mkdir();
		}
	...
	private void doStatus(HttpSession session, HttpServletResponse response)
		...
		if (fileUploadStats != null) {
			...
			// 将上传状态返回给客户端
			response.getWriter().println("<b>文件下载状态:</b><br/>");
			if (fileUploadStats.getBytesRead() != fileUploadStats
				...
				response.getWriter().println(
						"已下载: " + bytesProcessed + "bytes 总大小:" + sizeTotal
				...
				response.getWriter().println(
						"共下载: " + bytesProcessed + " bytes 总大小:" + sizeTotal
								+ " bytes<br/>");
				response.getWriter().println("文件下载完毕.<br/>");
			}
		}
		// 如果文件已经上传完毕
		if (fileUploadStats != null
				&& fileUploadStats.getBytesRead() == fileUploadStats
						.getTotalSize()) {
			response.getWriter().println("<b>Download complete.</b>");
		}
	}
